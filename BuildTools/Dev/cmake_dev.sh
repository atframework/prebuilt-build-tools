...
BUILD_DIR=$(echo "build-job-$SYS_NAME" | tr '[:upper:]' '[:lower:]');
...
while getopts "ac:d:e:hm:o:tusw-" OPTION; do
    case $OPTION in
        a)
        ...
        h)
            ...
            echo "-w                            using emsdk to build wasm.";
            exit 0;
        ;;
        m)
            if [ ! -z "$OPTARG" ]; then
        ...
        w)
            ...
            CXX=$(which em++);
            CMAKE_CLANG_ANALYZER=0;
            BUILD_DIR="build-job-wsl_emscripten";
        ;;
        -)
            break;
        ;;
...
if [ 0 -ne $EMSDK ]; then
    emcmake cmake .. $CMAKE_OPTIONS -DPROTOBUF_ROOT="$SCRIPT_DIR/thirdparty/protobuf/prebuilt/wsl_emscripten" -DPROTOBUF_HOST_BIN="$SCRIPT_DIR/thirdparty/protobuf/prebuilt/unix64/bin" "$@";
...
if [ 0 -eq $EMSDK ] && [ 1 -eq $CMAKE_CLANG_ANALYZER ]; then
    echo "=========================================================================================================";
    ...
    if [ -z "$CMAKE_CLANG_ANALYZER_PATH" ]; then
        echo "cd '$SCRIPT_DIR/$BUILD_DIR' && scan-build -o report --html-title='libmt_corestatic analysis' $CMAKE_CLANG_ANALYZER_OPTIONS make -j4";
    else
