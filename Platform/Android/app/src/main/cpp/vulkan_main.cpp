...
void CreateVulkanDevice(ANativeWindow* platformWindow, VkApplicationInfo* appInfo) {
    ...
    VkAndroidSurfaceCreateInfoKHR createInfo{
        .sType = VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR, .pNext = nullptr, .flags = 0, .window = platformWindow};
...
bool CreateBuffers(void) {
    ...
    MapMemoryTypeToIndex(memReq.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
...
void DeleteBuffers(void) { vkDestroyBuffer(device.device_, buffers.vertexBuf_, nullptr); }
...
VkResult CreateGraphicsPipeline(void) {
    ...
    VkResult pipelineResult =
        vkCreateGraphicsPipelines(device.device_, gfxPipeline.cache_, 1, &pipelineCreateInfo, nullptr, &gfxPipeline.pipeline_);
...
bool InitVulkan(android_app* app) {
    ...
    for (int bufferIndex = 0; bufferIndex < swapchain.swapchainLength_; bufferIndex++) {
        ...
        setImageLayout(render.cmdBuffer_[bufferIndex], swapchain.displayImages_[bufferIndex], VK_IMAGE_LAYOUT_UNDEFINED,
